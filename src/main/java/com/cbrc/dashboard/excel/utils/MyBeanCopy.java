...
public class MyBeanCopy {
	...
	public static <T> List<T> convert(List<Object> sources, Class<T> targetClass) {
	...
	public static <T> Map<String,List<T>> convert(Map<String,List<Object>> sources, Class<T> targetClass) {
		Map<String,List<Object>> sourcesObj = sources;
		if (sourcesObj == null) {
			sourcesObj = Collections.emptyMap();
		}
		Map<String,List<T>> targets = new HashMap<>();
		MyBeanCopy.convert(sourcesObj, targets, targetClass);
		return targets;
	}

	private static <T> void convert(Map<String,List<Object>> sources,Map<String,List<T>> targets, Class<T> targetClass) {
		if (targets == null) {
			return;
		}
		targets.clear();
		...
		for (Map.Entry<String, List<Object>> entry : sources.entrySet()) {
			try {
	...
	private static <T> void convert(List<Object> sources, List<T> targets, Class<T> targetClass) {
		if (targets == null) {
			return;
		}
		targets.clear();
